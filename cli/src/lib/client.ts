 ...
 * (`lib/scan.ts` runScan, `lib/report-fetch.ts` fetchLatestReportRest), and
 * the same client the production-verified `mcp-server/` package uses. No
  ...
  try {
    ...
    if (!Array.isArray(rows) || rows.length === 0) {
      return {
        ok: false,
        notFound: true,
        error: `No cached report exists yet for ${owner}/${repo}. Run \`claude-rabbit scan ${owner}/${repo}\` to create one.`,
      };
    }
