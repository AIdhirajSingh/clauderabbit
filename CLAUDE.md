...
1. **Wrapper first** — Next.js app, port the Claude Design UI, Supabase auth + database + edge functions, report rendering, cache, and the Gemini fast-path proving the end-to-end pipe (paste → edge function → model reads/scores → report renders).
...
**In V1:** paste-a-URL → fast safety verdict, free and unlimited with no login required; two-speed analysis with the dynamic sandbox working well enough on a meaningful share of real repos; public SEO-indexed `/owner/repo` reports with trust badge; cache by commit SHA + owner-cached reputation; verdicts that never say a bare "Safe"; homepage with live scans, viral repos, and the dangerous-repos leaderboard.

**Login / limits / monetization:** scanning is free and unlimited, no login or ad ever required — a settled decision, not a placeholder. An earlier login-wall-plus-rewarded-ad gate (matching what this section used to say) was built, then deliberately removed in a real, reviewed commit after it was recognized as fake scaffolding — a demo ad with a "Skip ad (demo)" button, not a real ad integration — that contradicted the product's own no-fabricated-data rail. Signing in is optional and only saves scan history; it never blocks a scan or a result, fresh or cached. There is no daily scan quota (an earlier per-login/device daily-limit plan is also removed); a burst/velocity rate limit still exists purely as an anti-abuse floor against a request flood, never as a cap on legitimate daily usage. **This currently leaves the product with no monetization mechanism at all** — a real, open gap against "self-sustaining without debt," not a solved problem. See `docs/INFRASTRUCTURE.md` §9 and the cost/unit-economics doc for the honest current numbers this leaves.

**Distribution surfaces — shipped, now real (scope expansion beyond the original V1 plan, authorized by direct instruction as "the primary growth channel for a free ad-free product"):** a real **MCP server** (`mcp-server/`, one cache-aware `scan` tool over stdio — also served remotely over Streamable HTTP at `/mcp` for claude.ai custom connectors — calling the production Supabase scan API) and a real **CLI** (`cli/`, package `clauderabbit`, published to npm, with `scan`/`mcp install`/`login`/`logout` subcommands). Both require a signed-in ClaudeRabbit account (a real product/access decision distinct from the web app's own no-login rule above — the scan data itself isn't sensitive; report pages stay public either way). These are built, in the repo, and reuse the same server-side scan pipeline — they are not roadmap. Each honors the same rails as the web app: honest verdict, never a bare "Safe." (A Claude Code plugin approach was tried and dropped — the MCP server and CLI are the only supported distribution surfaces.) An opt-in shell-hook feature (`install-hooks`/`uninstall-hooks`, `npm-install`/`pnpm-install`/`git-clone`) was built and then removed in a real, reviewed commit after recognizing a fundamental design gap: it scanned a package's linked GitHub repo, not the actual published registry artifact, so it could not catch the exact attack an install-time check most needs to (a compromised maintainer publishing a malicious version directly to the registry). It will be planned and built properly against the real published artifact, not shipped with that gap.
